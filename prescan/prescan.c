...
static void alloc_buffers_and_user_data(cl_context context)
{
  ...
  B_buffer = clCreateBuffer(context,  CL_MEM_READ_WRITE,  sizeof(int)  * SIZE, NULL, NULL);
  if (!B_buffer)
...
static void check_output_data(void)
{
  ...
  for(int i = 1; i < SIZE; i++) {
    int expected = A_data[i] + A_data[i-1];
...
int main(int argc, char** argv)
{
  ...
  for (cl_int p = 0; p < nb_platforms; p++) {
	...
	size_t local = TILE ;   // local domain size for our calculation
	size_t shared;
	int group_index=0;
	int base_index=0;
	unsigned int count = TILE;
	...
	err |= clSetKernelArg(kernel, 2, shared , 0);
       	err |= clSetKernelArg(kernel, 3, sizeof(cl_int), &count);
	err |= clSetKernelArg(kernel, 4, sizeof(cl_int), &base_index);
	err |= clSetKernelArg(kernel, 5, sizeof(cl_int), &count);
