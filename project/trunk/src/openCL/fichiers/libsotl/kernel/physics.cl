...
__kernel
void scan (__global const int *box_buff,
	   __global int *calc_offset_buffer,
	   const int n)
{

	
	
        __local int temp[2048];  // allocated on invocation  
        int thid = get_local_id(0);
    	int offset = 1;  
	

    	temp[2*thid] = box_buff[2*thid]; // load input into shared memory  
    	temp[2*thid+1] = box_buff[2*thid+1];  
	 ...
	 calc_offset_buffer[2*thid-1] = temp[2*thid]; // write results to device memory  
	 calc_offset_buffer[2*thid] = temp[2*thid+1];  
	 
     	 barrier(CLK_GLOBAL_MEM_FENCE);

	 calc_offset_buffer[n-1] = temp[n-2] + box_buff[n-1];
}
